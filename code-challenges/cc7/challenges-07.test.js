...
const forEachTwoToThe = (arr) => {
  let results = [];
  arr.forEach(element => {
    results.push(Math.pow(2, element));
  });
...
const charCode = (arr) => {
  let results = [];
  results = (arr.map(string => string.charCodeAt()));
  console.log(results);
  return (results);
};
...
const extractAbilities = (arr) => {
  // console.log('extractAbilities:', arr.abilities[0].ability.name);
  let results = [];
  arr.map((n) => {
    results.push(n.ability.name);
  });
  return (results);
};
...
const extractStats = (arr) => {
  let results = [];
  arr.map((n) => {
    results.push({name: n.stat.name, total: n.effort + n.baseStat});
  });
  console.log(results);
  return (results);
};
